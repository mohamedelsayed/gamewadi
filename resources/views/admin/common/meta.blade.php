<head>
    ...
    <title>Admin Panel |
        @if(!empty($result['commonContent']['setting']['seo_title']))
        <?= stripslashes($result['commonContent']['setting']['seo_title']) ?>
        @endif
    </title>
    {{--{{ $pageTitle }}--}}
    @if(!empty($result['commonContent']['setting']['favicon']))
    <link rel="icon" type="image/png" href="{{asset('').$result['commonContent']['setting']['favicon']}}">
    @endif
